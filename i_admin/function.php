			...
			// this is for create calendra and view add event and view event and number of Events
				...
				for($cb=1;$cb<=$boxDisplay;$cb++){
					if(($cb >= $current_Month_First_Day+1 || $current_Month_First_Day == 7) && $cb <= ($total_Days_ofMonthDisplay)){
						...
						$result = $mysqli->query("SELECT page_title FROM events WHERE event_date = '".$currentDate."'");
...
function get_events_information($date = ''){
	...
	$result = $mysqli->query("SELECT * FROM events WHERE event_date = '".$date."'");
...
function add_event_information($date,$title,$mmsg){
	...
	$insert = $mysqli->query("INSERT INTO events (page_title,details,event_date,posted_on,update_on) VALUES ('".$title."','".$mmsg."','".$date."','".$currentDate."','".$currentDate."')");
	if($insert){
		echo 'ok';
	}else{
		echo $mysqli->error;
