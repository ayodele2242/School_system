...
/*$db = new PDO('mysql:dbname=wesley', 'root', '',
...
$select = $db->prepare('SELECT `id` FROM `navigation_bar` WHERE `level` = :level');
$update = $db->prepare('UPDATE `navigation_bar` SET `level` = :level WHERE `parent_id` = :parent');
...
// prepare a query to get subitems of the current $item (those with parent = $item)
    $subsel = $db->prepare('SELECT *  FROM navigation_bar WHERE parent_id = :parent ORDER BY `position`');
// run the query
...
// will recursively call createsubmenu to add possibly existing subitems
        $text .= '<li class="menucolor' . ($i->level - 1) . '">'
            .'<a href="' . htmlspecialchars($i->link) . '">' . htmlspecialchars($i->name) . '</a>'
